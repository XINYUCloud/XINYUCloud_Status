...
class Session {
    ...
    public static function start(): void { if(self::$started||php_sapi_name()==='cli'){self::$started=true;return;} $r=RedisClient::getInstance(); if($r->isConnected()){ini_set('session.save_handler','redis'); $auth=REDIS_PASS!==''?'?auth='.urlencode(REDIS_PASS).'&':'?'; ini_set('session.save_path','tcp://'.REDIS_HOST.':'.REDIS_PORT.$auth.'prefix='.REDIS_PREFIX.'session:');}else{ini_set('session.save_handler','files'); $sp=session_save_path()?:sys_get_temp_dir();ini_set('session.save_path',$sp);} ini_set('session.use_strict_mode','1'); ini_set('session.use_only_cookies','1'); ini_set('session.cookie_httponly','1'); ini_set('session.cookie_samesite','Lax'); ini_set('session.gc_maxlifetime',(string)SESSION_LIFETIME); session_name(SESSION_NAME); if(session_status()===PHP_SESSION_NONE)@session_start(); self::$started=true; if(!isset($_SESSION['_last_regenerated']))self::regenerate(); elseif(time()-$_SESSION['_last_regenerated']>1800)self::regenerate(); }
